...
class SingleEpisodeGenerator:
    ...
    def sample(self, length: int, rng: np.random.Generator | None = None, vehicle: VehicleCapabilities | None = None, initial_speed: float | None = None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        single_config = GeneratorConfig(
            ...
            rate_max_limit=self.batch_config.rate_max_limit,
            rate_neg_max_limit=self.batch_config.rate_neg_max_limit,
            lpf_safety_factor=self.batch_config.lpf_safety_factor,
...
def create_reference_generator(
    ...
    if generator_config is not None:
        config = GeneratorConfig(
            ...
            rate_max_limit=generator_config.get('rate_max_limit', 15.0),
            rate_neg_max_limit=generator_config.get('rate_neg_max_limit', 20.0),
            lpf_safety_factor=generator_config.get('lpf_safety_factor', 0.85),
        ...
        config = GeneratorConfig(
            ...
            jerk_max=12.0,
            rate_max_limit=15.0,
            rate_neg_max_limit=20.0,
            lpf_safety_factor=0.85,
            p_change=0.03,
            p_zero_stop=0.08
        )

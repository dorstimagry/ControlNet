...
class TripDatasetParser:
    ...
    def _load_trip(self, trip_dir: Path) -> List[dict[str, np.ndarray]]:
        ...
        # Derived signals -------------------------------------------------
        data["throttle"] = self._fix_actuator_offset(data["throttle"])
        data["brake"] = self._fix_actuator_offset(data["brake"])
    ...
    def _load_sensor(self, path: Path, column: str) -> tuple[np.ndarray, np.ndarray] | None:
        ...
        if column not in df or "time_stamp" not in df:
            return None
        ts = pd.to_numeric(df["time_stamp"], errors="coerce").to_numpy(dtype=np.float64, copy=False)
    ...
    def _fix_actuator_offset(self, signal: np.ndarray) -> np.ndarray:
        mask = signal > 100.0
        if not np.any(mask):
            return signal
        corrected = signal.copy()
        corrected[mask] -= 108.47458
        return corrected
